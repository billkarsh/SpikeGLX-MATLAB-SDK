...
{
    if( nlhs != 1 ) {
        mexErrMsgTxt( "createNewClient: Returns handle to LHS." );
    }

    if( nrhs != 2 ) {
        mexErrMsgTxt( "createNewClient: Requires RHS: host, port." );
    }
    ...
    if( !mxIsChar( host ) || mxGetM( host ) != 1 )
        mexErrMsgTxt( "createNewClient: Hostname must be a string row vector." );

    if( !mxIsDouble( port )
        || mxGetM( port ) != 1
        || mxGetN( port ) != 1 ) {

        mexErrMsgTxt( "createNewClient: Port must be a scalar numeric value." );
    }
    ...
    const mxArray   *prhs[] )
{
    ...
    ulong   datalen = ulong(mxGetM( prhs[1] ) * mxGetN( prhs[1] ));
    ...
    switch( mxGetClassID( prhs[1] ) ) {
        ...
        case mxDOUBLE_CLASS:
            ...
            break;
        default:
            mexErrMsgTxt( "sendMatrix: Sent matrix must have numeric type." );
    }
...
{
    if( nlhs < 1 )
        mexErrMsgTxt( "readLine: Returns string to LHS." );
...
{
    if( nlhs < 1 )
        mexErrMsgTxt( "queryString: Returns string to LHS." );
...
{
    if( nlhs < 1 )
        mexErrMsgTxt( "getCells: Returns cells{} to LHS." );
    ...
    try {
        ...
        if( m ) {
            ...
            for( int i = 0; i < m; ++i )
                mxSetCell( plhs[0], i, mxCreateString( &vlines[i][0] ) );
        }
    }
    catch( const exception &e ) {
...
// ----------------
    ...
    const char  *str;
    int         buflen = int(mxGetM( prhs[1] ) * mxGetN( prhs[1] )) + 1;
...
// --------------------------
    ...
    if( !_strcmpi( str, "double" ) ) {
        ...
        mexErrMsgTxt(
        "readMatrix: Output matrix type must be one of {"
...
// ---------------------------------------
    ...
    if( !cmd2fun.size() ) {
        ...
        cmd2fun["querystring"]                  = queryString;
        cmd2fun["getcells"]                     = getCells;
